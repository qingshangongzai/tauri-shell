...
Rust 层几乎零逻辑（仅拖放示例的文件大小命令与系统托盘两处能力；托盘默认不打包、由 HTML 标记按需开启），故名「**轻壳**」。这一命名的架构定位辨析（为什么需要“轻壳”、何时该选 Web 方向）见 [docs/关于“轻壳”架构的定位辨析.md](docs/关于“轻壳”架构的定位辨析.md)。
...
> 🤖 **不会写代码也能用**：把 [docs/AI生成页面需求模板.md](docs/AI生成页面需求模板.md) 发给 AI，附一句需求（如"生成一个食品保质期计算器"）即可得到符合规范的页面；想要同风格完整应用骨架（无边框标题栏已内置），也可以把 `components/starter.html` 发给 AI。两种都无需改任何配置，详见使用说明「标题栏」章节。
...
│   ├── src/
│   │   ├── main.rs         ← 入口（无需修改）
│   │   └── lib.rs          ← Tauri 轻壳（拖放示例的文件大小命令 + 托盘（HTML 标记开启，见 build.rs））
│   ├── capabilities/
...
│   ├── build.rs            ← 构建脚本（无需修改；自动按 data-tauri-tray 标记裁剪托盘代码）
...
├── minify.cjs              ← HTML 压缩（构建时自动调用，按标记过滤托盘菜单页）
...
- 不存在 → 自动使用系统原生标题栏。托盘默认不打包：点 X 直接退出进程；`<html>` 加 `data-tauri-tray` 标记才打包托盘（点 X 最小化到托盘，托盘右键菜单退出）。
...
托盘**默认不打包**（无托盘图标、无菜单窗口，关闭主窗口直接退出进程）；需要托盘时，
在 `dist/index.html` 的 `<html>` 上加上 `data-tauri-tray` 属性并重新构建即可——
构建期自动打包：关闭窗口最小化到托盘（设置页「窗口」卡片可关，关闭后点 X 直接退出）、
托盘左键还原主窗口，右键弹出与应用同风格的自绘菜单（透明 Webview 小窗渲染，非原生菜单）。
标记读取发生在构建期，必须静态写在 HTML 文件里（运行时渲染无效）；示例页与
`components/starter.html` 已自带该标记。

另外内置 `tauri-plugin-single-instance` 单实例保护：窗口驻留托盘时再次启动 exe
不会产生第二个进程（多托盘图标），而是还原已有实例。移除托盘后可保留（对常规单实例
同样有效），需要从依赖层面彻底移除时见 [docs/使用说明.md](docs/使用说明.md)「系统托盘」章「彻底移除」。

工作原理、菜单项定制、注意事项与构建期开关的自动化测试（`npm run test:tray`）见
[docs/使用说明.md](docs/使用说明.md) 中的「系统托盘」章节。
