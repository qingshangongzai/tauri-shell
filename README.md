...
Rust 层几乎零逻辑（仅拖放示例的文件大小命令与系统托盘两处能力，均可按需移除），故名「**轻壳**」。
...
### 4️⃣ 不想装环境？用 GitHub Actions 云构建

本地零环境也能出安装包：

**fork 仓库 → 网页端把 `dist/index.html` 换成你的页面 → 发一个 `v*` 标签的 Release → 等云端构建完成，安装包自动附在 Release 里。**
