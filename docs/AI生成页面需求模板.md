...
- 单个功能的简单应用无需导航、**无需卡片包裹**：组件（表单、结果区等）直接平铺在 `.content` 里，用间距表达结构，不套多余的容器层级。内容块默认水平居中（骨架 `.content-inner > *` 已内置限宽居中），不要让内容靠左。
...
> **仲裁原则**：本规范各条规则与其例外条件冲突、或无法明确判断是否属于例外时，一律执行主规则——宁少分割线、宁保守，不自行发挥例外。

1. **间距**：模块与组件间的留白以 8px 为基准——同一信息组内 8/16px，不同信息组之间 24/32px，页面章节/模块之间 48/64px，页面边距 16/24px；控件内边距、字号等微观尺寸沿用骨架既有值（如 2/4/6/10/12px），不受 8px 约束，其他尺寸一律按 8px 标尺就近取整。
2. **无边框原则**：表达分隔与层级用**留白、色块、弥散阴影**，不用边框线；仅两类场景可保留分割线——①**高密度表格**：以行列网格展示数据且**行数 ≥ 10、列数 ≥ 3** 的表格/数据列表；②**警示场景**：警告区、删除确认、错误提示。卡片列表、表单、设置项不算表格类，一律去线。
...
5. **动效**：时长只用 `--duration-fast`（100ms）/ `--duration-normal`（200ms）/ `--duration-slow`（300ms），缓动统一 `var(--ease)`；微交互 100–150ms 取 `--duration-fast`、弹层 200–250ms 取 `--duration-normal`、页面转场 250–350ms 取 `--duration-slow`（区间内数值一律就近取令牌，不另设时长）；必须带 `prefers-reduced-motion` 降级。
...
- **层级克制**：能平铺就不套容器——功能简单的页面直接放组件，用间距表达结构；卡片/容器仅用于区分多个功能模块。多余的容器层级与分割线同理，都是视觉噪音。
...
- **高密度数据界面（表格类）**：指以行列网格展示数据且**行数 ≥ 10、列数 ≥ 3** 的表格或数据列表；卡片列表、表单、设置项不算。可保留分割线，或采用斑马纹（交替极浅底色）辅助快速扫描。
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* 内容块水平居中：简单页面内容不靠左（限宽 + 自动外边距） */
        .content-inner > * { max-width: 960px; margin-left: auto; margin-right: auto; }
